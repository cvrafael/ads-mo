...
module.exports = {
  async createPost(req, res) {
    try {
      ...
      // const { id } = req.params;
      const { title, description, image, fk_id_user_entity } = req.body;
      ...
      if (post != "") {
        ...
        `INSERT INTO "public"."posts" ("title","description","image","fk_id_user_entity") VALUES ('${title}','${description}','${image}','${fk_id_user_entity}');`,
  ...
  async findUserPosts(req, res) {
    try {
      ...
      const userAllPosts = await db.query(
        `SELECT pp.id, pp.title, pp.description, pp.image, pp.fk_id_user_entity, pp."timestamp" FROM public.posts as pp
